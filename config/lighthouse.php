...
return [
    ...
    */
    ...
    'route' => [
         ...
         * We include Sanctum's stateful middleware so session cookies from the Nuxt SPA
         * are recognized, along with WAF rate limiting and HTTP security headers.
         */
        'middleware' => [
    ...
    */
    ...
    /*
     * We explicitly support both 'sanctum' (for SPA session cookies / bearer tokens)
     * and 'web' (for standard web sessions) when authenticating GraphQL requests.
     */
    'guards' => ['sanctum', 'web'],
    ...
    */
    ...
    /*
     * Enforce strict query depth and complexity limits. Depth is capped at 8 to prevent
     * deeply nested denial-of-service queries (e.g. infinite post->comment->post chains).
     * Complexity is capped at 200 to protect server resources under RoadRunner workers.
     */
    'security' => [
        'max_query_complexity' => (int) env('LIGHTHOUSE_MAX_QUERY_COMPLEXITY', 200),
        'max_query_depth' => (int) env('LIGHTHOUSE_MAX_QUERY_DEPTH', 8),
